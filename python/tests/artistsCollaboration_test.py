"""Unit tests for the ArtistsCollaboration contract classes.

"""

import smartpy as sp

# Import the artistsCollaboration module
artistsCollaborationModule = sp.io.import_script_from_url(
    "file:python/contracts/artistsCollaboration.py")
...
minterModule = sp.io.import_script_from_url("file:python/contracts/minter.py")
marketplaceModule = sp.io.import_script_from_url("file:python/contracts/marketplace.py")
...
def get_test_environment():
    ...
    # Initialize the artists collaboration originator contract
    originator = artistsCollaborationModule.ArtistsCollaborationOriginator(
        metadata=sp.utils.metadata_of_url("ipfs://aaa"))

    # Initialize the lambda provider contract
    lambda_provider = artistsCollaborationModule.LambdaProvider(
...
def test_full_example():
    ...
    operators_params_type = sp.TList(sp.TVariant(
        add_operator=fa2Module.FA2.OPERATOR_KEY_TYPE,
        remove_operator=fa2Module.FA2.OPERATOR_KEY_TYPE))
    swap_params_type = sp.TRecord(
        token_id=sp.TNat,
        editions=sp.TNat,
        price=sp.TMutez,
        donations=sp.TList(
            marketplaceModule.Marketplace.ORG_DONATION_TYPE)).layout(
