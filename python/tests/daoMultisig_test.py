"""Unit tests for the DAO multisig wallet contract class.

"""

import smartpy as sp

# Import the DAO multisig and fa2 contract modules
daoMultisigModule = sp.io.import_script_from_url("file:contracts/daoMultisig.py")
...
def get_test_environment():
    ...
    # Initialize the DAO multisig contract
    multisig = daoMultisigModule.DaoMultisig(
    ...
    # The users accept the membership
    multisig.accept_membership(True).run(sender=user1)
    multisig.accept_membership(True).run(sender=user2)
    multisig.accept_membership(True).run(sender=user3)
    multisig.accept_membership(True).run(sender=user4)
...
def test_create_vote_and_execute_proposal():
    ...
    # Check that the new user is not official a user yet, but it has been added
...
def test_add_user_proposal():
    ...
    # Check that the user was added to the list of proposed users
    scenario.verify(sp.len(multisig.data.users.elements()) == 4)
...
@sp.add_test(name="Test accept membership")
def test_accept_membership():
    ...
    multisig.add_user_proposal(user6.address).run(sender=user4)
    ...
    # Execute the proposal
    multisig.execute_proposal(1).run(sender=user3)
    ...
    multisig.accept_membership(True).run(valid=False, sender=user4)
    multisig.accept_membership(False).run(valid=False, sender=user4)
    multisig.accept_membership(True).run(sender=user5)
    multisig.accept_membership(False).run(sender=user6)
...
def test_leave_multisig():
    ...
    # User 4 leaves the multisig
    multisig.leave_multisig().run(sender=user4)

    # Check that the user was removed from the multisig
    scenario.verify(sp.len(multisig.data.users.elements()) == 3)
    scenario.verify(~multisig.data.users.contains(user4.address))
    scenario.verify(~multisig.is_user(user4.address))
    scenario.verify(multisig.data.minimum_votes == 3)

    # User 3 also leaves the multisig
    multisig.leave_multisig().run(sender=user3)

    # Check that the user was removed from the multisig
    scenario.verify(sp.len(multisig.data.users.elements()) == 2)
    scenario.verify(~multisig.data.users.contains(user3.address))
    scenario.verify(~multisig.is_user(user3.address))

    # Check that the minimum votes parameter has been updated
    scenario.verify(multisig.data.minimum_votes == 2)

    # User 2 also leaves
    multisig.leave_multisig().run(sender=user2)

    # Check that the user was removed from the multisig
    scenario.verify(sp.len(multisig.data.users.elements()) == 1)
    scenario.verify(~multisig.data.users.contains(user2.address))
    scenario.verify(~multisig.is_user(user2.address))
    scenario.verify(multisig.data.minimum_votes == 1)

    # User 1 cannot leave, because they are the last member
    multisig.leave_multisig().run(valid=False, sender=user1)
