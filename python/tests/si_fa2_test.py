...
# Import the Spanish Inquisition fa2 contract module
fa2Module = sp.io.import_script_from_url("file:contracts/si_fa2.py")
...
def get_test_environment():
    ...
    fa2 = fa2Module.SpanishInquisitionFA2(
...
@sp.add_test(name="Test transfer with allow lists")
def test_transfer_with_allow_lists():
    # Get the test environment
    testEnvironment = get_test_environment()
    scenario = testEnvironment["scenario"]
    admin = testEnvironment["admin"]
    ...
    # Check that the contract information has been updated
    scenario.verify(fa2.get_balance(sp.record(owner=user1.address, token_id=0)) == editions - 6)
    scenario.verify(fa2.get_balance(sp.record(owner=user2.address, token_id=0)) == 3)
    scenario.verify(fa2.get_balance(sp.record(owner=user3.address, token_id=0)) == 3)
    scenario.verify(fa2.total_supply(0) == editions)

    # Create two allow lists
    allow_list_1 = listModule.SpanishInquisitionList(
        administrator=user1.address,
        metadata=sp.utils.metadata_of_url("ipfs://aaa"),
        type=sp.variant("ALLOW", sp.unit))
    allow_list_2 = listModule.SpanishInquisitionList(
        administrator=user2.address,
        metadata=sp.utils.metadata_of_url("ipfs://aaa"),
        type=sp.variant("ALLOW", sp.unit))
    scenario += allow_list_1
    scenario += allow_list_2

    # Fill the allow lists
    allow_list_1.add_members([user1.address]).run(sender=user1)
    allow_list_2.add_members([user1.address, user3.address]).run(sender=user2)

    # Check that only the token admin can add or remove lists
    fa2.update_lists(sp.variant("add_allow_list", allow_list_1.address)).run(
        valid=False, sender=user1, exception="FA2_NOT_ADMIN")

    # Add the allow lists to the token contract
    fa2.update_lists(sp.variant("add_allow_list", allow_list_1.address)).run(sender=admin)
    fa2.update_lists(sp.variant("add_allow_list", allow_list_2.address)).run(sender=admin)

    # Check that the contract information has been updated
    scenario.verify(fa2.data.allow_lists.contains(allow_list_1.address))
    scenario.verify(fa2.data.allow_lists.contains(allow_list_2.address))

    # Check that the second user cannot send or receive tokens because they are
    # not in the allow lists
    fa2.transfer([
        sp.record(
            from_=user2.address,
            txs=[sp.record(to_=user3.address, token_id=0, amount=1)])
        ]).run(valid=False, sender=user2, exception="FA2_NOT_ALLOWED_USER")
    fa2.transfer([
        sp.record(
            from_=user3.address,
            txs=[sp.record(to_=user2.address, token_id=0, amount=1)])
        ]).run(valid=False, sender=user3, exception="FA2_NOT_ALLOWED_USER")

    # Check that the third user can send tokens to the first user because they
    ...
    # Check that the contract information has been updated
    scenario.verify(fa2.get_balance(sp.record(owner=user1.address, token_id=0)) == editions - 6 + 1)
    scenario.verify(fa2.get_balance(sp.record(owner=user2.address, token_id=0)) == 3)
    scenario.verify(fa2.get_balance(sp.record(owner=user3.address, token_id=0)) == 3 - 1)
    scenario.verify(fa2.total_supply(0) == editions)
    ...
    # Check that the second user cannot transfer the user1 editions because they
    ...
    # Make the first user as operator of the second user token
    fa2.update_operators([sp.variant("add_operator", sp.record(
        owner=user2.address,
        operator=user1.address,
        token_id=0))]).run(sender=user2)

    # Check that the first user cannot transfer the user2 editions because they
    # are not in the allow list
    fa2.transfer([
        sp.record(
            from_=user2.address,
            txs=[sp.record(to_=user3.address, token_id=0, amount=5)])
        ]).run(valid=False, sender=user1, exception="FA2_NOT_ALLOWED_USER")

    # Remove one of the allow lists
    fa2.update_lists(sp.variant("remove_allow_list", allow_list_2.address)).run(sender=admin)

    # Check that the contract information has been updated
    scenario.verify(fa2.data.allow_lists.contains(allow_list_1.address))
    scenario.verify(~fa2.data.allow_lists.contains(allow_list_2.address))

    # Check that the third user cannot send tokens to the first user because
    # they are not the allow list
    fa2.transfer([
        sp.record(
            from_=user3.address,
            txs=[sp.record(to_=user1.address, token_id=0, amount=1)])
...
@sp.add_test(name="Test transfer with block lists")
def test_transfer_with_block_lists():
    # Get the test environment
    testEnvironment = get_test_environment()
    scenario = testEnvironment["scenario"]
    admin = testEnvironment["admin"]
    ...
    # Create two block lists
    block_list_1 = listModule.SpanishInquisitionList(
        administrator=user1.address,
        metadata=sp.utils.metadata_of_url("ipfs://aaa"),
        type=sp.variant("BLOCK", sp.unit))
    block_list_2 = listModule.SpanishInquisitionList(
        administrator=user2.address,
        metadata=sp.utils.metadata_of_url("ipfs://aaa"),
        type=sp.variant("BLOCK", sp.unit))
    scenario += block_list_1
    scenario += block_list_2

    # Fill the block lists
    block_list_1.add_members([user1.address]).run(sender=user1)
    block_list_2.add_members([user1.address, user3.address]).run(sender=user2)

    # Check that only the token admin can add or remove lists
    fa2.update_lists(sp.variant("add_block_list", block_list_1.address)).run(
        valid=False, sender=user1, exception="FA2_NOT_ADMIN")

    # Add the allow lists to the token contract
    fa2.update_lists(sp.variant("add_block_list", block_list_1.address)).run(sender=admin)
    fa2.update_lists(sp.variant("add_block_list", block_list_2.address)).run(sender=admin)

    # Check that the contract information has been updated
    scenario.verify(fa2.data.block_lists.contains(block_list_1.address))
    scenario.verify(fa2.data.block_lists.contains(block_list_2.address))

    # Check that the first and third users cannot send or receive tokens because
    # they are in the block lists
    fa2.transfer([
        sp.record(
            from_=user1.address,
            txs=[sp.record(to_=user2.address, token_id=0, amount=1)])
        ]).run(valid=False, sender=user1, exception="FA2_BLOCKED_USER")
    fa2.transfer([
        sp.record(
            from_=user3.address,
            txs=[sp.record(to_=user2.address, token_id=0, amount=1)])
        ]).run(valid=False, sender=user3, exception="FA2_BLOCKED_USER")

    # Check that the second user can send tokens to the admin because they
    # are not in the block list
    fa2.transfer([
        sp.record(
            from_=user2.address,
            txs=[sp.record(to_=admin.address, token_id=0, amount=1)])
        ]).run(sender=user2)

    # Check that the contract information has been updated
    scenario.verify(fa2.get_balance(sp.record(owner=user1.address, token_id=0)) == editions - 6)
    scenario.verify(fa2.get_balance(sp.record(owner=user2.address, token_id=0)) == 3 - 1)
    scenario.verify(fa2.get_balance(sp.record(owner=user3.address, token_id=0)) == 3)
    scenario.verify(fa2.get_balance(sp.record(owner=admin.address, token_id=0)) == 1)
    scenario.verify(fa2.total_supply(0) == editions)
    ...
    fa2.transfer([
        sp.record(
            from_=user1.address,
            txs=[sp.record(to_=user3.address, token_id=0, amount=5)])
        ]).run(valid=False, sender=user2, exception="FA2_BLOCKED_USER")
    ...
    # Remove one of the allow lists
    fa2.update_lists(sp.variant("remove_block_list", block_list_2.address)).run(sender=admin)

    # Check that the contract information has been updated
    scenario.verify(fa2.data.block_lists.contains(block_list_1.address))
    scenario.verify(~fa2.data.block_lists.contains(block_list_2.address))

    # Check that the third user can send tokens because they are not in the
    # block lists
    fa2.transfer([
        sp.record(
            from_=user3.address,
